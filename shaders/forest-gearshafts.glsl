...
float sceneSDF(vec3 p)
{
    float scene = 0.0;
    const float PI = 3.1415926;
    p = Ry(iGlobalTime/8.)*p;
    p = Rz(iGlobalTime/8.)*p;

    p += vec3(iGlobalTime/4., 0.0, 0.0);

    p = Rx(0.3)*p;

    vec3 c = vec3(.55, 0.0, 1.1);
    p = opRep(p, c);
    vec3 d = vec3(0.15, 0.01, 0.1);
    float r = 0.025;
    float o = 0.1;

    p = Rz(iGlobalTime)*p;

    float boxes = roundBox(vec3(p.x+o, p.y, p.z), d, r);
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z+.8), d, r));
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z-.8), d, r));
    float hex = hexPrism(vec3(p.x, p.y, p.z), vec2(o, 1.0));
    p = Rz(PI/3.)*p;
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z+.6), d, r));
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z-.6), d, r));
    p = Rz(PI/3.)*p;
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z+.6), d, r));
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z-.6), d, r));
    p = Rz(PI/3.)*p;
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z), d, r));
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z+.6), d, r));
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z-.6), d, r));
    p = Rz(PI/3.)*p;
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z+.6), d, r));
    ...
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z+.6), d, r));
    boxes = unionSDF(boxes, roundBox(vec3(p.x+o, p.y, p.z-.6), d, r));
    scene = polynomialUnionSDF(boxes, hex, 0.02);
...
{
    vec3 ambientLight = 0.25 * vec3(1.0, 1.0, 1.0);
...
void main()
{
    ...
    {
          float bg = cos(uv.x) + sin(uv.y);
          gl_FragColor = vec4(0.0, bg/5., bg/10., 0.0);
		      return;
    }
    ...
    vec3 K_a = (estimateNormal(p) + vec3(1.0)) / 5.0;
