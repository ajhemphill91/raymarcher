...
#define NEAR_CLIP_PLANE         2.0
#define FAR_CLIP_PLANE          20.0
#define MAX_RAYCAST_STEPS       50
#define STEP_DAMPING            0.9
#define DIST_EPSILON            0.1
#define MAX_RAY_BOUNCES         8.0

#define MAX_SHADOW_DIST         5.0
#define SHADOW_K								64.0

#define AMBIENT_COLOR           vec3(0.0, 0.0, 0.0)
...
vec2 map(vec3 p)
{
    vec2 scene;
		...
		float box0 = fBox(p + vec3(0.0, 0.0, -6.0), vec3(6.0, 10.0, 0.1));
		float box1 = fBox(p + vec3(0.0, 0.0, 10.0), vec3(6.0, 10.0, 0.1));
		float box2 = fBox(Rx(PI/2.)*p + vec3(0.0, 2.0, 10.0), vec3(6.0, 8.0, 0.1));
		float box3 = fBox(Ry(PI/2.)*p + vec3(-2.0, 0.0, 6.0), vec3(8.0, 10.0, 0.1));
		float box4 = fBox(Ry(PI/2.)*p + vec3(-2.0, 0.0, -4.0), vec3(8.0, 10.0, 0.1));
		float boxes = smoothUnionSDF(box0, box1, r_c);
		boxes = smoothUnionSDF(boxes, box2, r_c);
		boxes = smoothUnionSDF(boxes, box3, r_c);
		boxes = smoothUnionSDF(boxes, box4, r_c);
		float sphere0 = fTorus(Rx(iGlobalTime)*Ry(iGlobalTime)*(p + vec3(sin(iGlobalTime), sin(iGlobalTime), sint(4.0, 2.0))), 0.5, 1.0);

		scene = vec2(boxes, MTL_2);
		scene = unionSDF(scene, vec2(sphere0, MTL_1));
    return scene;
}
...
vec3 getMaterialColor(float matID)
{
    vec3 col = CLR_BACKGROUND;
		if (matID <= MTL_1) 					col = vec3(0.1, 0.3, 0.9);
		else if(matID <= MTL_2)				col = vec3(1.0, 0.1, 0.1);
...
void main(void)
{
    ...
    float ang = 0.3*sin(iGlobalTime/4.);
