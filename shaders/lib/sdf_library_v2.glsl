...
//saturate is reserved after glsl-preview 0.16, really shitty thing to do yo
...
#define MAX_RAY_BOUNCES         10.0

#define MAX_SHADOW_DIST         0.0
...
#define SPEC_POWER             	32.0		//Incident light transmitted power, I
...
vec3 getMaterialColor(float matID)
{
    vec3 col = CLR_BACKGROUND;
		if (matID <= MTL_1) 					col = vec3(0.9, 0.3, 0.1);
...
vec3 render(vec3 rayOrig, vec3 rayDir)
{
    ...
    for (float i = 0.0; i < MAX_RAY_BOUNCES; i++)
        ...
        vec3 col = mtlDiffuse*(AMBIENT_COLOR + LIGHT_COLOR*diffuse + fresColor);
				//vec3 col = mtlDiffuse*(AMBIENT_COLOR + LIGHT_COLOR*(diffuse + specular*SPEC_COLOR)); //Blinn-phong speculars
        col = applyFog(col, t);
