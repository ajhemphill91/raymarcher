...
struct Hit_t
{
    ...
    float materialID;			//ID that yields a Material_t for this point
};
...
vec2 map(vec3 p)
{
    vec2 scene;
		...
		float box = fBox(Rz(iGlobalTime/2.+0.5)*p, vec3(1.0));
		float box1 = fBox(p + vec3(0.0, 10.0, 0.0), vec3(15.0, 0.5, 15.0));

		//The sphere will be a point light
		float sphere = fSphere(Ry(PI/2.)*p + vec3(6.0, -8.0, 2.0), 0.5);

		scene = vec2(box, MTL_1);
		scene = unionSDF(scene, vec2(box1, MTL_MIRROR));
		scene = unionSDF(scene, vec2(sphere, MTL_EMISSION));
...
Material_t getMaterial(float matID)
{
		...
		if(matID == MTL_1)
		{
				m.diffuseColor = vec3(0.9, 0.3, 0.1);
				m.smoothness = 0.0;
		}
		else if(matID == MTL_2)
		{
				m.diffuseColor = vec3(1.0, 0.1, 0.1);
		}
		else if(matID == MTL_MIRROR)
		{
				m.diffuseColor = vec3(0.2, 0.1, 0.3);
...
vec3 computeColor(Ray_t ray, Hit_t hit, Material_t mat)
{
		...
		for (int j = 0; j < NUM_LIGHTS; j++)
		{
				...
				{
					bpi = blinnPhongIntensity(ray.rd, lightRay.rd, hit.N, mat.smoothness) *
						_lights[j].intensity / (ray.totalDistance * ray.totalDistance);	//ray.rd is used here as the view vector
				}



				//vec3 fresColor = SPEC_COLOR + (1.0 - SPEC_COLOR)*pow((1.0 - specular), 5.0); //Schlick approximation for fresnel speculars
				//lambertian *= shadow(secondary.ro, lightDir);
...
vec3 render(Ray_t ray)
{
    ...
    float alpha = 1.0;
		...
		//Operate off a secondary ray so we can compare against the original (where ray.ro is the camera)
		Ray_t secondary = ray;
    ...
    for (float i = 0.0; i < MAX_RAY_BOUNCES; i++)
        ...
        vec2 marchingResult = rayMarch(secondary);
				Hit_t hit;
				hit.distTravelled = marchingResult.x;
				secondary.totalDistance += hit.distTravelled;
				hit.materialID = marchingResult.y;
				hit.pos = secondary.ro + (secondary.rd*hit.distTravelled);
        hit.N = normal(hit.pos);
        hit.R = reflect(secondary.rd, hit.N);
				...
				{
						Ray_t tertiary;
						tertiary.wasTransmitted = true;
						hit.T = refract(secondary.rd, hit.N, 1.0/mat.n);
						tertiary.ro = hit.pos;
						tertiary.rd = hit.T;
						vec2 transmissionResult = rayMarch(tertiary);
						Hit_t tHit;
						tHit.distTravelled = transmissionResult.x;
						tertiary.totalDistance += tHit.distTravelled;
						tHit.materialID = transmissionResult.y;
						tHit.pos = tertiary.ro + (tertiary.rd*tHit.distTravelled);
		        tHit.N = normal(tHit.pos);
						Material_t tMat = getMaterial(tHit.materialID);

						resCol += computeColor(tertiary, tHit, tMat);

						continue;

				}

				resCol += computeColor(secondary, hit, mat);
        ...
        if (mat.maxReflections == 0.0 || abs(dot(hit.N, secondary.rd)) < 0.01 ||
						hit.materialID == MTL_BACKGROUND) break;
