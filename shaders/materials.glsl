...
vec4 map(vec3 p)
{
    vec4 scene;
		...
		vec3 p0 = p;
		p0 = Rx(iGlobalTime)*p0;
		float sphere0 = fSphere(p0, 1.0);
		sphere0 -= 0.1*noise(p0.xz);
		vec3 sphere0kd = vec3(0.3, 0.3, 0.3)*noise(p.xz)*Rx(iGlobalTime)*p.zyx*20.;

		float sphere1 = fSphere(p0 + vec3(-2.5, 0.0, 0.0), 1.0);
		sphere1 -= 0.1*noise(p0.yz);
		vec3 sphere1kd = vec3(1.0, 0.9, 3.0)*noise(p.yz*40.);

		float sphere2 = fSphere(p0 + vec3(2.5, 0.0, 0.0), 1.0);
		sphere2 -= 0.1*noise(p0.xy);
		vec3 sphere2kd = vec3(1.0)*noise(simplexNoise().xx)*2.;

		p = Rx(PI/6.)*p;
		float bg = fBox(p + vec3(0.0, 4.0, 10.0), vec3(10.0, 1.0, 10.0));

		scene = vec4(sphere0, sphere0kd);
		scene = union4SDF(scene, vec4(sphere1, sphere1kd));
		scene = union4SDF(scene, vec4(sphere2, sphere2kd));
		scene = union4SDF(scene, vec4(bg, 0.7, 0.8, 0.8));
...
{
    vec3 ambientLight = 0.1 * vec3(1.0, 1.0, 1.0);
    vec3 color = ambientLight * k_a;

    vec3 light1Pos = eye + vec3(0.0, 15.0, 0.0);
...
void main()
{
    ...
    vec3 eye = vec3(-2.5, -2.5, 15.0);
    vec3 center = vec3(-2.5, -2.5, 0.0);
    ...
    vec4 m = map(p);
		vec3 n = estimateNormal(p);
		vec3 lightPos = eye + vec3(0.0, 15.0, 0.0);
		vec3 lightDir = viewToWorld*normalize(lightPos);
		float shadow = 0.0;
    vec3 K_a, K_d, K_s;
    float shininess = 10.0;

		K_d = vec3(m.y, m.z, m.w);
		K_a = vec3(0.4);
		K_s = vec3(1.0);

		//Light the scene
    vec3 color = phongIllumination(K_a, K_d, K_s, shininess, p, eye);
		shadow = softshadow(eye, worldDir, 64.0);
		color -= shadow;
