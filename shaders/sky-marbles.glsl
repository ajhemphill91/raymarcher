...
{
    p -= vec3(1.0, 1.0, 0.0);
    p = Rz(iGlobalTime/10.)*p;
    vec3 c = vec3(10.0);
    p = opRep(p, c);
    float scene = sphere(p, 1.0);
...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float shininess, vec3 p, vec3 eye)
{
    ...
    light1Pos = Rx(iGlobalTime)*light1Pos;
...
void main()
{
    ...
    vec3 distColor = unpackColor(dist);
    ...
    if (dist > MAX_DIST - EPSILON)
    {
          ...
          gl_FragColor = vec4(0.0, 0.0, bg/4.0, 0.0);
		      return;
    }
    ...
    vec3 K_a = vec3(0.0);//(estimateNormal(p) + vec3(1.0)) / 2.0;
    vec3 K_d = texture2D(texture, uv).rgb;
    ...
    gl_FragColor = vec4(distColor.r, distColor.g, distColor.b, 1.0);
