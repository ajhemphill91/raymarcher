...
vec3 random3(vec3 p)
{
		p = vec3( dot(p, vec3(127.1, 311.7, 269.5)),
							dot(p, vec3(287.8, 75.3, 194.8)),
							dot(p, vec3(447.1, 119.7, 249.5)));
...
float noise(vec3 p)
{
    ...
    return mix( mix( dot(random3(i), f),
...
#define CAM_DIST                180.0
#define CAM_H                   1.0
#define CAM_FOV_FACTOR 					4.0
#define LOOK_AT_H               4.0
#define LOOK_AT 								vec3(0.0, LOOK_AT_H, 0.0)

#define MTL_BACKGROUND          100.0
#define MTL_SUN									1.0
#define MTL_MERCURY							2.0
#define MTL_VENUS								3.0
#define MTL_EARTH								4.0
#define MTL_MARS								5.0
#define MTL_JUPITER							6.0
#define MTL_SATURN							7.0
#define MTL_RINGS								8.0
#define MTL_URANUS							9.0
#define MTL_NEPTUNE							10.0
#define MTL_MIRROR							11.0

#define SCALE										0.015
#define PLANET_SCALE						2.0
#define R_EARTH									1.0*SCALE			//Actual radius is 6,371.0 km
#define R_SUN 									109.0*R_EARTH/PLANET_SCALE
#define R_MERCURY								0.38*R_EARTH
#define R_VENUS									0.95*R_EARTH
#define R_MARS									0.53*R_EARTH
#define R_JUPITER								11.19*R_EARTH
#define R_SATURN								9.40*R_EARTH
#define R_I_RINGS								4.0*1.10*R_EARTH + R_SATURN
#define R_O_RINGS								10.0*12.56*R_EARTH + R_I_RINGS
#define R_URANUS								4.04*R_EARTH
#define R_NEPTUNE								3.88*R_EARTH

#define O_SCALE									1.0*SCALE
#define O_MERCURY								10958.54*R_EARTH*O_SCALE
#define O_VENUS									17099.19*R_EARTH*O_SCALE
#define O_EARTH									23873.0*R_EARTH*O_SCALE
#define O_MARS									39114.74*R_EARTH*O_SCALE
#define O_JUPITER								128086.64*R_EARTH*O_SCALE
#define O_SATURN								236854.49*R_EARTH*O_SCALE
#define O_URANUS								472139.38*R_EARTH*O_SCALE
#define O_NEPTUNE								712603.99*R_EARTH*O_SCALE

#define ORBITAL_SPEED						3.0
#define PERIOD_EARTH						1.0*ORBITAL_SPEED
#define PERIOD_MERCURY					0.24*PERIOD_EARTH
#define PERIOD_VENUS						0.62*PERIOD_EARTH
#define PERIOD_MARS							1.88*PERIOD_EARTH
#define PERIOD_JUPITER					11.86*PERIOD_EARTH
#define PERIOD_SATURN						29.46*PERIOD_EARTH
#define PERIOD_URANUS						84.02*PERIOD_EARTH
#define PERIOD_NEPTUNE					164.8*PERIOD_EARTH

#define CLR_BACKGROUND 					vec3(0.025, 0.025, 0.075)

vec2 map(vec3 p)
{
    vec2 scene;
		float t = iGlobalTime;
		vec3 c = (p + vec3(0.0, -7.0, 0.0));
		float sun = fSphere(c, R_SUN);
		sun -= sin(16.*c.x)/420. + sin(137.*c.y)/370. + sin(251.*c.z)/302.;

		float mercury = fSphere(Rz(t/PERIOD_MERCURY)*c + vec3(0.0, O_MERCURY, 0.0), R_MERCURY);
		float venus = fSphere(Rz(t/PERIOD_VENUS)*c + vec3(0.0, O_VENUS, 0.0), R_VENUS);
		float earth = fSphere(Rz(t/PERIOD_EARTH)*c + vec3(0.0, O_EARTH, 0.0), R_EARTH);
		float mars = fSphere(Rz(t/PERIOD_MARS)*c + vec3(0.0, O_MARS, 0.0), R_MARS);
		float jupiter = fSphere(Rz(t/PERIOD_JUPITER)*c + vec3(0.0, O_JUPITER, 0.0), R_JUPITER);
		float saturn = fSphere(Rz(t/PERIOD_SATURN)*c + vec3(0.0, O_SATURN, 0.0), R_SATURN);
		float rings = fTorus(Rz(t/PERIOD_SATURN)* + vec3(0.0, O_SATURN, 0.0), R_I_RINGS, R_O_RINGS);
		float uranus = fSphere(Rz(t/PERIOD_URANUS)*c + vec3(0.0, O_URANUS, 0.0), R_URANUS);
		float neptune = fSphere(Rz(t/PERIOD_NEPTUNE)*c + vec3(0.0, O_NEPTUNE, 0.0), R_NEPTUNE);

		float mirror = fBox(c + vec3(0.0, 0.0, 5.0), vec3(2.0*O_NEPTUNE, 2.0*O_NEPTUNE, 0.5));

		//TODO: Build scene here
		scene = vec2(sun, MTL_SUN);
		scene = unionSDF(scene, vec2(mercury, MTL_MERCURY));
		scene = unionSDF(scene, vec2(venus, MTL_VENUS));
		scene = unionSDF(scene, vec2(earth, MTL_EARTH));
		scene = unionSDF(scene, vec2(mars, MTL_MARS));
		scene = unionSDF(scene, vec2(jupiter, MTL_JUPITER));
		scene = unionSDF(scene, vec2(saturn, MTL_SATURN));
		scene = unionSDF(scene, vec2(rings, MTL_RINGS));
		scene = unionSDF(scene, vec2(uranus, MTL_URANUS));
		scene = unionSDF(scene, vec2(neptune, MTL_NEPTUNE));
		scene = unionSDF(scene, vec2(mirror, MTL_MIRROR));
    return scene;
}
...
vec3 getMaterialColor(float matID)
{
    vec3 col = CLR_BACKGROUND;
		if (matID <= MTL_SUN) 					col = vec3(0.9, 0.3, 0.1);
		else if(matID <= MTL_MERCURY)		col = vec3(1.0, 0.1, 0.1);
		else if(matID <= MTL_VENUS)			col = vec3(0.9, 0.3, 0.2);
		else if(matID <= MTL_EARTH)			col = vec3(0.0, 0.3, 0.9);
		else if(matID <= MTL_MARS)			col = vec3(0.9, 0.2, 0.3);
		else if(matID <= MTL_JUPITER)		col = vec3(0.9, 0.3, 0.3);
		else if(matID <= MTL_SATURN)		col = vec3(0.7, 0.7, 0.7);
		else if(matID <= MTL_RINGS)			col = vec3(0.9, 0.9, 0.3);
		else if(matID <= MTL_URANUS)		col = vec3(0.2, 0.8, 0.3);
		else if(matID <= MTL_NEPTUNE)		col = vec3(0.1, 0.0, 1.0);
...
vec3 render(vec3 rayOrig, vec3 rayDir)
{
    ...
    for (float i = 0.0; i < MAX_RAY_BOUNCES; i++)
        ...
        if (mtlID < MTL_MIRROR || abs(dot(nrm, rayDir)) < 0.1) break;
...
void main(void)
{
    ...
    float ang = 2.0*(sin(iGlobalTime/3.0)*0.5+1.0);
