...
vec4 sphere(vec3 p, float r, float id)
{
    const float PI = 3.1415926
    float s = acos(p.z/r)/PI;
    float t = acos(r*sin(s*PI))/(2*PI);
    return vec4(length(p)-r, id, s, t);
}
...
/**
    The scene
      ...
      return.z is the texCoord x
      return.w is the texCoord y
*/
...
uniform sampler2D moon_norm;

uniform sampler2D moss;
uniform sampler2D moss_norm;

uniform sampler2D stones_smooth;
uniform sampler2D stones_smooth_norm;
