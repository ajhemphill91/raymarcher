...
vec2 map(vec3 p)
{
    vec2 scene;
		...
		vec3 pg = p;
		pg.z += 200.;
		float ground = fBox(pg + vec3(0.0, 25.0, 0.0), vec3(1000.0, 10.0, 10000.0));
		ground += sin(pg.x/pg.z) + cos(pg.z/4.);
		ground += 10.0*noise(pg.xz*0.1);



		//TODO: Build scene here
		p = Ry(iGlobalTime/4.)*p;
		pModIntervalMirror1(p.y, 1.0, 1.0, 2.0);
		pModIntervalMirror1(p.y, 2.0, 2.0, 2.0);
		pModIntervalMirror1(p.y, 4.0, 3.0, 2.0);

		//pMod1(p.x, 12.0);
		pModPolar(p.xz, 25.);
		pR(p.zx, iGlobalTime/8.);
		pR(p.xy, iGlobalTime/8.);
		pMirrorOctant(p.xz, vec2(1.0, 1.25));
		pR(p.zy, iGlobalTime/8.);
		pMirrorOctant(p.yz, vec2(1.25, 1.0));
		float box = fBox(p, vec3(3.0, 3.0, 0.5));
		vec3 p_c = p;
		pMod3(p_c, vec3(2.0, 0.0, 0.05));
		box = fOpGroove(box, fCylinder(p_c, 0.5, 3.0), 0.25, 0.25);

		vec3 p_w = p - vec3(0.0, 1.0, 0.0);
		float window = fCylinder(p_w, 0.0, 1.25);
		vec2 h = vec2(0.75, 0.25);
		vec3 o = Rx(PI/2.0)*vec3(1.0, 0.0, 0.0);
		vec3 p_hex = Rx(PI/2.0)*p;
		float hex = fHexagonIncircle(p_hex, h);
		hex = fOpUnionChamfer(hex, fHexagonIncircle(p_hex-o, h), 0.25);
		box = fOpDifferenceChamfer(box, window, 0.9);
		box = fOpUnionColumns(box, fBlob(p), 0.9, 3.0);

		scene = unionSDF(vec2(box, 2.0), vec2(ground, 1.0));
...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float shininess, vec3 p, vec3 eye)
{
    ...
    vec3 light1Pos = eye + vec3(0.0, 15.0, 1.0);
...
void main()
{
    ...
    vec3 viewDir = rayDirection(45.0, iResolution.xy, gl_FragCoord.st);
		...
		float s = 50.0;
    vec3 eye = vec3(0.0, 0.0, s);
    vec3 center = vec3(-0.175*s, -0.175*s, 0.0);
    ...
    if(m.y <= 1.0)
    {
				...
				K_a = vec3(0.7, 0.8, 0.7);
        K_d = (estimateNormal(p) + vec3(1.0))/2.;
				K_d *= vec3(0.6, 0.9, 0.2)*noise(uv/1.5);
        K_s = vec3(0.1);
        shininess = 1.;
    }
    else if(m.y <= 2.0)
    {
        ...
        K_a = vec3(0.3);
        K_d = unpackColor(dist*100000.).bgr*1.5;
        K_s = vec3(1.0);
        shininess = 10.;
    }
    ...
    vec3 color = phongIllumination(K_a, K_d, K_s, shininess, p, eye);
		float shadow = softshadow(eye, worldDir, 32.0);
		color *= shadow;

		vec3 lightDir = normalize(p - eye + vec3(0.0, 15.0, 1.0));
