...
vec2 map(vec3 p)
{
    ...
    vec3 p_h = (p + vec3(0.0, 0.0, 99.0));

    p = Rx(sin(iGlobalTime/4.2))*p;
    p = opRep(p, vec3(12.0));

    //lamps
    scene = vec2(roundBox(p, fw, 0.1), 1.0);
    vec2 shell = vec2(sphere(p, 1.4), 3.0);
    shell = intersectSDF(shell, vec2(roundBox(Ry(PI/-2.0)*p-vec3(0.0, 0.0, 2.0), fw*2., 0.1), 3.0));
    p = Ry(iGlobalTime/2.0)*p;
    scene = unionSDF(scene, vec2(sphere(p, 0.9), 2.0));
    scene = unionSDF(scene, shell);

    p_h.z -= length(cross(p_h.zzz, p_h));
    vec2 hex = vec2(box(p_h, vec3(40.0)), 4.0);

    scene = unionSDF(scene, hex);
...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float shininess, vec3 p, vec3 eye)
{
    ...
    vec3 light1Pos = eye + vec3(-iGlobalTime, 1000.0, 0.0);
...
void main()
{
    ...
    float shadow = softshadow(eye, worldDir, 2.0);

    if(m.y <= 1.0)
    {
        K_a = unpackColor(dist);
        K_d = K_a;
        K_s = vec3(1.0);
        shininess = 20.;
    }
    ...
    {
        K_a = unpackColor(pow(dist, sinp(iGlobalTime/2., 2.5, 2.0)));
        K_d = K_a;
        K_s = vec3(1.0);
        shininess = 1.;
    }
    ...
    {
        K_a = vec3(0.1, 0.2, 0.5);
        K_d = unpackColor(dist*200000.);
